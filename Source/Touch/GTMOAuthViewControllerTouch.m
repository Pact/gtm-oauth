...
+ (BOOL)authorizeFromKeychainForName:(NSString *)appServiceName
    ...
    NSString *userID = [keychain userIDForService:appServiceName
...
+ (BOOL)removeParamsFromKeychainForName:(NSString *)appServiceName {
    GTMOAuthKeychain *keychain = [GTMOAuthKeychain defaultKeychain];
    [keychain removeUserIDForService:appServiceName
                             account:kGTLOAuthAccountName
                               error:nil];
...
+ (BOOL)saveParamsToKeychainForName:(NSString *)appServiceName
    ...
    GTMOAuthKeychain *keychain = [GTMOAuthKeychain defaultKeychain];
    [keychain setUserID:auth.userID
...
#pragma mark UserID
// iPhone
- (NSString *)userIDForService:(NSString *)service account:(NSString *)account error:(NSError **)error {
    OSStatus status = kGTLOAuthKeychainErrorBadArguments;
    NSString *result = nil;
    if (0 < [service length] && 0 < [account length]) {
        CFDataRef data = NULL;
        NSString *accountForUserID = [NSString stringWithFormat:@"%@: UserID", account];
        NSMutableDictionary *keychainQuery = [self keychainQueryForService:service account:accountForUserID];
        [keychainQuery setObject:(id)kCFBooleanTrue forKey:(id)kSecReturnData];
        [keychainQuery setObject:(id)kSecMatchLimitOne forKey:(id)kSecMatchLimit];

        status = SecItemCopyMatching((CFDictionaryRef)keychainQuery,
                                     (CFTypeRef *)&data);
        if (status == noErr && 0 < [(NSData *)data length]) {
...
// iPhone
- (BOOL)removeUserIDForService:(NSString *)service account:(NSString *)account error:(NSError **)error {
    OSStatus status = kGTLOAuthKeychainErrorBadArguments;
    if (0 < [service length] && 0 < [account length]) {
        NSString *accountForUserID = [NSString stringWithFormat:@"%@: UserID", account];
        NSMutableDictionary *keychainQuery = [self keychainQueryForService:service account:accountForUserID];
